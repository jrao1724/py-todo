...
class TodoItem:
    """ Class definition for each todo item, which is defined with todo -a, and stored in the datafile """
    ...
    def __str__(self):
        ...
        if days_left == 0:
            ...
            if config['PY-TODO'].getboolean('detail_mode', fallback=False):
                ...
                #     (2 - 2) / 7 = 0 ... 0
                #
                # Therefore, apply ceiling function to weeks_left
    ...
    if len(items) > 0:
        ...
        for index, item in enumerate(items):
            ...
            if '(' in item_str:
                ...
                # The date could possibly be either color, so no issue in calling maybe_color_str multiple times
    ...
    if len(sys.argv) <= 1:
        ...
        if len(sys.argv) >= 3:
            try:
                ...
                # If the input is canceled, print a newline for prettiness, list the items and exit
                # Don't need to re-add the old item here because if input is canceled, nothing actually changed it
                print()
                list_items()
                sys.exit()
        else:
